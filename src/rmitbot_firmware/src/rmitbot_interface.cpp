...
RmitbotInterface::~RmitbotInterface() {
  if (arduino_.IsOpen()) {
    try {
      ...
      RCLCPP_FATAL_STREAM(
          rclcpp::get_logger("RmitbotInterface"),
...
CallbackReturn RmitbotInterface::on_init(
  ...
  try {
    ...
    RCLCPP_FATAL(rclcpp::get_logger("RmitbotInterface"),
...
CallbackReturn
RmitbotInterface::on_deactivate(const rclcpp_lifecycle::State &) {
  RCLCPP_INFO(rclcpp::get_logger("RmitbotInterface"),
...
RmitbotInterface::export_state_interfaces() {
  ...
  // Linear acceleration
  state_interfaces.emplace_back(imu_name, "linear_acceleration.x",
                                &lin_acc_[0]);
  state_interfaces.emplace_back(imu_name, "linear_acceleration.y",
                                &lin_acc_[1]);
  state_interfaces.emplace_back(imu_name, "linear_acceleration.z",
                                &lin_acc_[2]);
...
RmitbotInterface::export_command_interfaces() {
...
RmitbotInterface::read(const rclcpp::Time &, const rclcpp::Duration &) {
  // Interpret the string from Arduino, e.g.: "<1.24\t-2.36>"
  // It is much better to use starting and ending delimiters, because serial
  // will fail Usually because Arduino is sending too fast, or the buffer is not
  // empty
  if (arduino_.IsDataAvailable()) {
    auto dt = (rclcpp::Clock().now() - last_run_).seconds();
    std::string message;
    arduino_.ReadLine(message);
    // RCLCPP_INFO(rclcpp::get_logger("RmitbotInterface"), "Message Received:
    // %s", message.c_str());
    ...
    message.erase(std::remove(message.begin(), message.end(), '\r'),
                  message.end());
    // RCLCPP_INFO(rclcpp::get_logger("RmitbotInterface"), "Trimmed message:
    // '%s'", message.c_str());
    ...
    if (!message.empty() && message.front() == '<' && message.back() == '>') {
      ...
      // RCLCPP_INFO(rclcpp::get_logger("RmitbotInterface"), "Data Received:
      // %s", data.c_str());

      while (std::getline(ss, token, '\t')) {
        states_data.push_back(
            std::stof(token)); // Store the right_vel and left_vel in the vector
        // RCLCPP_INFO(rclcpp::get_logger("RmitbotInterface"), "Parsed
        // velocities: %.2f, %.2f", velocity_states_.at(0),
        // velocity_states_.at(1));
      }

      // Update each wheel state
      if (states_data.size() >= 4) {
      ...
      // Check if we have IMU data (needs 14 total elements: 4 wheels + 10 IMU)
...
// =======================================================================
hardware_interface::return_type
RmitbotInterface::write(const rclcpp::Time &, const rclcpp::Duration &) {
...
PLUGINLIB_EXPORT_CLASS(rmitbot_firmware::RmitbotInterface,
